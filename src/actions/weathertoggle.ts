...
const UUID = "com.luke-abel.deck-weather.weathertoggle";
 ...
 */
@action({ UUID })
export class WeatherToggle extends SingletonAction<WeatherSettings> {
	/**
	...
	override async onKeyDown(ev: KeyDownEvent<WeatherSettings>): Promise<void> {
		const settings = await streamDeck.settings.getGlobalSettings() as WeatherSettings;
		memoizeSettings(settingsMemo, settings);
		let promise = beginInterval(ev.action, settingsMemo.get().refreshTime);
		currentMeasurement = currentMeasurement === 'c' ? 'f' : 'c';
		return promise;
	}
}
...
function generateTitle(temp: number, humidity: number, windspeed: number): string {
	// If the current measurement needs to be converted, do so.
	if (currentMeasurement === 'c') {
		temp = (temp - 32) * 5 / 9; // Convert from Fahrenheit to Celsius
		windspeed = windspeed * 1.60934; // Convert from mph to km/h

		const roundedTemp = Math.round(temp || 0);
		const humidex = calculateHumidex(roundedTemp, humidity, false);
		return `${roundedTemp}°C, ${humidity}%\n\n\n\n~${humidex}°C`;
	} else {
		const roundedTemp = Math.round(temp || 0);
		const humidex = calculateHumidex(roundedTemp, humidity, true);
		return `${roundedTemp}°F, ${humidity}%\n\n\n\n~${humidex}°F`;
	}
}

/**
 * Calculates the humidex value from temperature and relative humidity.
 * @param temp Temperature value
 * @param humidity Relative humidity (0-100)
 * @param isFahrenheit If true, temp is in Fahrenheit; if false or omitted, temp is in Celsius
 * @returns Humidex value (rounded integer)
 */
function calculateHumidex(temp: number, humidity: number, isFahrenheit = false): number {
	// Convert Fahrenheit to Celsius if needed
	let tempC = isFahrenheit ? (temp - 32) * 5 / 9 : temp;
	// Step 1: Estimate dew point
	...
	// Step 3: Calculate humidex
	const humidexC = tempC + 0.5555 * (e - 10);
	// Convert humidex back to Fahrenheit if needed
	return isFahrenheit ? Math.round(humidexC * 9 / 5 + 32) : Math.round(humidexC);
}
